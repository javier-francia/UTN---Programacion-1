...
void* al_pop(ArrayList *pList, int index)
{
    ...
    if (pList != NULL && index >= 0 && index < pList->size)
    {
        if((returnAux = al_get(pList, index)) != NULL)
        {
...
ArrayList* al_subList(ArrayList *pList, int from, int to)
{
    ...
    if (pList != NULL)
    {
        if (from >= 0 && from < to && to <= pList->size)
        {
            returnAux = al_newArrayList();
    ...
    if(pList != NULL && pFunc != NULL)
    {
        if (order == 0 || order == 1)
        {
            ...
            for(int i = 0; i < (pList->size - 1); i++)
            {
                ...
                {
                    if (order == 1 && pFunc(pList->pElements[i], pList->pElements[j]) == 1)
                    {
                        swapAux = al_get(pList, i);
                        al_set(pList, i, al_get(pList, j));
                        al_set(pList, j, swapAux);
                    }
                    else if(order == 0 && pFunc(pList->pElements[i], pList->pElements[j]) == -1)
                    {
                        swapAux = al_get(pList, i);
                        al_set(pList, i, al_get(pList, j));
                        al_set(pList, j, swapAux);
...
int resizeUp(ArrayList *pList)
{
    ...
    if(pList != NULL)
    {
        if (pList->size >= pList->reservedSize)
        {
            ...
            ptrAux = realloc(pList->pElements, sizeof(void**) * (pList->reservedSize + AL_INCREMENT));
